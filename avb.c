/*
   ...
   2021-04-12 : Modified by Raphael Assenat for the Sega Master System (SDCC, devkitSMS/SMSlib)
   About the comment above: "Yep, make that four, and the situation is not improved ;-)"
...
#define N_MENU_ENTRIES	4
const char *menu[N_MENU_ENTRIES] = {
 {"   Human vs. Human   "},
 {" Human vs. Computer  "},
 {" Computer vs. Human  "},
...
#define TID_NET_TOP_L	141
#define TID_NET_TOP_R	(TID_NET_TOP_L+1)

#define TID_NET_L	157
#define TID_NET_R	(TID_NET_L + 1)
...
void main()
{
	...
	SMS_loadBGPalette(main_pal);
	SMS_loadSpritePalette(main_pal);
	SMS_loadTiles(main_tiles, 0, main_tiles_size);
	...
	// Player left 4,5,6,7,8,9,10,11,12
...
#error Broken code
 ...
 if (reprint) {
   ...
   */
   ...
   SMS_setNextTileatXY(MENU_X, MENU_Y + i);
   printf("%c %s", i == opt ? '*':' ', menu[i]);
  }
  reprint=0;
 }
...
static void move3x3_metasprite(uint8_t id, uint8_t X, uint8_t Y)
{
 SMS_updateSpritePosition(id++, X, Y);
 SMS_updateSpritePosition(id++, X + 8, Y);
 SMS_updateSpritePosition(id++, X + 16, Y);
 SMS_updateSpritePosition(id++, X, Y + 8);
 SMS_updateSpritePosition(id++, X + 8, Y + 8);
 SMS_updateSpritePosition(id++, X + 16, Y + 8);
 SMS_updateSpritePosition(id++, X, Y + 16);
 SMS_updateSpritePosition(id++, X + 8, Y + 16);
 SMS_updateSpritePosition(id++, X + 16, Y + 16);
}

static void move2x2_metasprite(uint8_t id, uint8_t X, uint8_t Y)
{
 SMS_updateSpritePosition(id++, X, Y);
 SMS_updateSpritePosition(id++, X + 8, Y);
 SMS_updateSpritePosition(id++, X, Y + 8);
...
void putshapes()
{
	...
	const uint8_t pright_tiles[9] = { 144, 145, 146,	160, 161, 162,	176, 177, 178 };
	...
	*/
	...
	tmpx = XCNV(tbx);
	tmpy = YCNV(tby);
	move2x2_metasprite(0, tmpx, tmpy);

	ball_frm = ((tbx/16)&3)*2;
	SMS_updateSpriteImage(0, 192 + ball_frm);
	SMS_updateSpriteImage(1, 193 + ball_frm);
	SMS_updateSpriteImage(2, 208 + ball_frm);
	SMS_updateSpriteImage(3, 209 + ball_frm);
	...
	if (y[1] < 173) { // Jumping?
	 ...
	 pright_frm = ((x[1]/8)&1)*3;
	}

	// Player
	for (i=0; i<9; i++) {
	 SMS_updateSpriteImage(4 + i, pleft_tiles[i] + pleft_frm);
	 SMS_updateSpriteImage(13 + i, pright_tiles[i] + pright_frm);
	}

	move3x3_metasprite(4, XCNV(x[0]), YCNV(y[0]));
...
//  Text(TitleRastPort,scrstr,2);
  SMS_setNextTileatXY(server ? (P2_SCORE_X + 1) : (P1_SCORE_X + 1), TITLE_Y);
  printf(scrstr);
 }
 else {
  if (!menuon) {
  ...
  if (server) {
   SMS_setNextTileatXY(P1_SCORE_X, TITLE_Y); printf(" ");
   SMS_setNextTileatXY(P2_SCORE_X, TITLE_Y); printf("*");
...
// *dmaconw=0x000F;
 ...
 SMS_setNextTileatXY(P1_SCORE_X,TITLE_Y);
 printf(" 00");
 SMS_setNextTileatXY(TITLE_X,TITLE_Y);

// printf("Arcade Volleyball");
 // Game name in another color..
 for (i=224; i<=239; i++) {
	 SMS_setTile(i);
 }

 SMS_setNextTileatXY(P2_SCORE_X,TITLE_Y);
 printf(" 00");
 ...
 // Net
 SMS_setNextTileatXY(NET_X, NET_TOP_Y);
 SMS_setTile(TID_NET_TOP_L);
 SMS_setTile(TID_NET_TOP_R);

 for (i=NET_TOP_Y + 1; i<22; i++) {
 	SMS_setNextTileatXY(NET_X, i);
 	SMS_setTile(TID_NET_L);
	SMS_setTile(TID_NET_R);
 }

 SMS_setNextTileatXY(NET_X, 22);
 SMS_setTile(TID_NET_BASE_L);
 SMS_setTile(TID_NET_BASE_R);
...
SMS_EMBED_SDSC_HEADER_AUTO_DATE(VERSION_MAJ, VERSION_MIN, "raphnet.", "Arcade Volleyball", "Arcade Volleyball ported to the SMS");
